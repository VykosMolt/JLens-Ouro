...
class APIError(RuntimeError):
    def __init__(self, msg: str, status: int | None = None, *, graphql_body=None,
                 response_completed: bool = False):
        super().__init__(msg)
        self.status = status  # HTTP status when the provider answered; None for network/other failures
        # Structured evidence is populated only after a complete GraphQL HTTP
        # response. It is decoded JSON, not the original wire bytes.
        self.graphql_body = copy.deepcopy(graphql_body)
...
def gql(query: str, variables=None, write: bool = False) -> dict:
    ...
    if not isinstance(body, dict) or "errors" in body or not isinstance(body.get("data"), dict):
        raise APIError("GraphQL error: " + _redact(json.dumps(body)[:400]), status=st,
                       graphql_body=body, response_completed=True)
    return body["data"]
