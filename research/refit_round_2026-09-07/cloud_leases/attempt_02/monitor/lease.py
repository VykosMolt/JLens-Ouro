...
def bind_pod(state, pod):
    ...
    if (pod.get("imageName") != state["image"] or type(pod.get("gpuCount")) is not int or pod["gpuCount"] != 1
            ...
            or pod["machine"].get("gpuDisplayName") not in ("RTX 4090", "NVIDIA GeForce RTX 4090")):
...
def self_test():
    ...
    with tempfile.TemporaryDirectory(prefix="jlens-lease-test-") as temporary, ExitStack() as stack:
        ...
        def fixture(*, phase="not_started", ready=True):
            ...
            state = make_plan(bundle, stamp(), balance=27.0, rate=0.25, now=clock[0])
        ...
        def sample_pod(state, **fields):
            return {"id": "pod-A", "name": state["name"], "machineId": "machine-A",
                    ...
                    "volumeInGb": 0, "costPerHr": 0.34,
                    "machine": {"gpuDisplayName": "RTX 4090"}, **fields}
        ...
        plan = make_plan(bundle, stamp(), balance=27.0, rate=0.25, prior_spend=2.0)
        ...
        observe_cost(root, stale, sample_pod(state, costPerHr=0.25))
