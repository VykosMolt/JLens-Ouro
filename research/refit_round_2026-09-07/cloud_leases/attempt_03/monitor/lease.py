...
def offer():
    query = """query Quote($id: String!) { gpuTypes(input:{id:$id}) {
      id memoryInGb lowestPrice(input:{gpuCount:1,secureCloud:false,totalDisk:120,
        minMemoryInGb:48,minVcpuCount:8,supportPublicIp:true}) {
        stockStatus uninterruptablePrice minMemory minVcpu } } }"""
    values = api.gql(query, {"id": GPU})["gpuTypes"]
    if len(values) != 1 or values[0]["id"] != GPU or values[0]["memoryInGb"] != 24:
        raise ValueError("requested GPU offer is missing")
    result = values[0]["lowestPrice"]
...
def change(root, action):
    with locked(root, ".state.lock"):
        ...
        previous_rate = number(state["all_in_rate"], "previous all-in rate")
        action(state)
        state["all_in_rate"] = max(previous_rate, HARD_RATE, number(state["all_in_rate"], "all-in rate"))
...
def bind_pod(state, pod):
    ...
    if selected_gpu not in ("NVIDIA GeForce RTX 3090", "NVIDIA GeForce RTX 4090"):
...
def create(args):
    ...
    with locked(LEDGER, ".lease_creation.lock"):
        ...
        request = {"cloudType": "COMMUNITY", "gpuCount": 1, "gpuTypeId": GPU,
                   ...
                   "volumeInGb": 0, "minVcpuCount": 8, "minMemoryInGb": 48,
...
def self_test():
    ...
    with tempfile.TemporaryDirectory(prefix="jlens-lease-test-") as temporary, ExitStack() as stack:
        ...
        quote = {"gpuTypes": [{"id": GPU, "memoryInGb": 24, "lowestPrice": {
            "stockStatus": "low", "uninterruptablePrice": discount_rate}}]}
        ...
        passed("offer quote requests the deployment's disk RAM CPU and public IP constraints",
               all(value in quoted_query for value in ("totalDisk:120", "minMemoryInGb:48", "minVcpuCount:8", "supportPublicIp:true"))
        ...
        passed("3090 price selection retains the original 4090 budget deadline basis",
               GPU == "NVIDIA GeForce RTX 3090" and MAX_GPU_RATE == 0.22
               and HARD_RATE == 0.34 + STORAGE_RATE
        ...
        for rate in (None, True, float("nan"), float("inf"), -1, 0, 0.35):
        ...
        root, state = fixture()
        passed("selected 3090 binds at the authorized 0.22 price",
               bind_pod(state, sample_pod(state))["actual_gpu_rate"] == 0.22)
        previous_gpu = {**state, "gpu_type_id": "NVIDIA GeForce RTX 4090", "max_gpu_rate": 0.34}
        for label in ("RTX 4090", "NVIDIA GeForce RTX 4090"):
            passed("historical 4090 binding remains valid " + label,
                   bind_pod(previous_gpu, sample_pod(previous_gpu, costPerHr=0.34,
                            machine={"gpuDisplayName": label}))["actual_gpu_rate"] == 0.34)
        rejects("wrong 4090 assignment cannot satisfy a 3090 lease", lambda: bind_pod(
            state, sample_pod(state, machine={"gpuDisplayName": "RTX 4090"})))
        rejects("wrong 3090 assignment cannot satisfy a historical 4090 lease", lambda: bind_pod(
            previous_gpu, sample_pod(previous_gpu, machine={"gpuDisplayName": "RTX 3090"})))
        ...
        rejects("assigned 0.23 rate exceeds the selected GPU ceiling", lambda: bind_pod(
            state, sample_pod(state, costPerHr=0.23)))
        too_expensive = json.loads(json.dumps(quote))
        too_expensive["gpuTypes"][0]["lowestPrice"]["uninterruptablePrice"] = 0.23
        with patch.object(api, "gql", return_value=too_expensive):
            rejects("quoted 0.23 rate exceeds the selected GPU ceiling", offer)
        for field, value in (("id", None), ("id", 12), ("id", ""), ("id", "../pod"),
                             ("machineId", None), ("machineId", ""), ("gpuCount", True),
                             ("containerDiskInGb", 120.0), ("volumeInGb", False),
                             ("costPerHr", True), ("costPerHr", 0.35), ("machine", None)):
        ...
        class Provider:
            ...
            def gql(self, query, variables=None, write=False):
                if "gpuTypes" in query:
                    return {"gpuTypes": [{"id": GPU, "memoryInGb": 24, "lowestPrice": {
                        "stockStatus": "low", "uninterruptablePrice": discount_rate}}]}
        ...
        for label, provider in (("normal", Provider()), ("halted", Provider(halt_on_arm=True)),
            ...
            with patch.object(sys.modules[__name__], "LEDGER", base / label), \
                ...
                if label == "normal":
                    previous = io._mkdir(base / label / "previous")
                    io._new_json(previous / "LEASE.json", {"status": "terminated", "lease_spend_upper_usd": 2.0})
                    create(args)
