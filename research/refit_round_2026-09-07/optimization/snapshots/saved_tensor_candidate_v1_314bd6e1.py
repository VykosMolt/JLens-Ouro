...
class ReplicatedSavedTensorHooks:
    ...
    def pack(self, tensor: torch.Tensor) -> PackedLane | torch.Tensor:
        ...
        equal = None
        # Do not infer equality from one sampled lane. Accumulate all tests on
        # the device and make a single host decision after every lane is tested.
        for lane_index in range(1, self.batch_size):
            comparison = (_bits(logical.select(axis, lane_index)) == first_bits).all()
            equal = comparison if equal is None else equal & comparison
