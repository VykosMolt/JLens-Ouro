...
def _release_unused_memory(torch):
    """Return freed allocator pages between quiescent benchmark phases.

    This changes no live tensor, matrix or memory allowance. Recompute the
    ordinary host/cgroup budget afterwards, even when trimming is unavailable
    or the allocator cannot release any pages.
    """
    before = _host_memory()
    collected = gc.collect()
    torch.cuda.empty_cache()
    trim_record = {"available": False, "return": None}
    try:
        libc = ctypes.CDLL(None)
    except OSError as error:
        trim_record["unavailable_reason"] = str(error)
    else:
        trim = getattr(libc, "malloc_trim", None)
        if trim is not None:
            trim.argtypes = [ctypes.c_size_t]
            trim.restype = ctypes.c_int
            trim_record = {"available": True, "return": int(trim(0))}
    return {"before": before, "after": _host_memory(), "gc_collected": collected,
            "malloc_trim": trim_record,
            "scope": "released unused allocator pages only; live tensors and host/cgroup guards unchanged"}
...
def execute(args, main_contract, prepared=None, prerequisites=None):
    ...
    runner._new_json(args.output_dir / "START.json", result)
    if args.kind == "huginn":
    ...
    for name, target, sources, paired in profiles:
        ...
        row["memory_cleanup"] = {"before_native": _release_unused_memory(torch)}
        budget = _offload_budget(paired=paired)
        row["host_offload_budget"] = budget
        runner._new_json(args.output_dir / f"{name}_START.json", row)
        ...
        row["memory_cleanup"]["after_native"] = _release_unused_memory(torch)
        details = {}
        torch.cuda.reset_peak_memory_stats()
        started = time.monotonic()
        if paired:
        ...
        del native
        row["memory_cleanup"]["before_io"] = _release_unused_memory(torch)
        ...
        del observed
        row["memory_cleanup"]["after_profile"] = _release_unused_memory(torch)
