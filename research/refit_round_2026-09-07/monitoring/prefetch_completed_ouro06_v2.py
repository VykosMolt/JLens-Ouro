"""V2: bounded transport retry/resume for 14 sealed Ouro/control binaries.
...
def resume_manifest(prior, expected):
    ...
    require(prior.parent == io._no_links(LEASE_ROOT) and prior.is_dir()
            and re.fullmatch(r'prefetch_(?:v2_)?[0-9]{8}T[0-9]{6}Z(?:_[0-9a-f]{8})?', prior.name),
    ...
    require(source in ({'bytes': 25901, 'sha256': V1_SOURCE_SHA256}, io._record(Path(__file__))),
...
def main(argv=None):
    ...
    if args.command == 'describe':
        ...
        return 0
    operation = io._no_links(LEASE_ROOT / ('prefetch_v2_' + datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ')
                                          + '_' + uuid.uuid4().hex[:8]))
