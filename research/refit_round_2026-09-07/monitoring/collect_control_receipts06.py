"""Copy small completed-control receipts; do not read any model or estimator binary."""
...
parser.add_argument('--profile', choices=('ouro_penultimate', 'ouro_positions'), required=True)
...
r = workspace / 'results'
fit = r / 'controls' / profile / 'fit_01'
log = r / 'logs/control_fits.log'
...
for line in log_bytes.splitlines():
    try:
        ...
        continue
    if isinstance(row, dict) and row.get('profile') == profile and row.get('status') == 'complete':
...
paths = [r / 'controls/RUN_IDENTITY.json', fit / 'OWNER.json', fit / 'LATEST.json', fit / 'COMPLETE.json']
for filename, parent, stdout_key, binary in [('LATEST.json', 'checkpoints', 'checkpoint_path', 'state.pt'),
                                            ('COMPLETE.json', 'final', 'lens_path', 'lens.pt')]:
    ...
    assert complete_lines[0][stdout_key] == str(generation / binary)
...
'''.replace('PROFILE_NAME', repr(args.profile))
...
destination = deployment.parent / 'monitoring/attempt_06' / f'{args.profile}_{stamp}'
...
lease.io._new_json(destination / 'COPY_VERIFIED.json', {
    ...
    'fit_id': 1, 'profile': args.profile, **captured,
    'scope': 'Eight immutable control-fit provenance JSONs and a captured log prefix; no estimator binaries read or independently verified, no recovery analysis, and not final experiment retrieval.',
})
