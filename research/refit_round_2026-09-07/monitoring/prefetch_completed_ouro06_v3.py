...
MAX_ATTEMPTS = 5
V1_SOURCE_SHA256 = '3f6bdbdabff8523da8ae23c67439daa336766e1bf38a2982adbdd73f9090ad25'
RSYNC_TRANSPORT_CODES = {10, 12, 30, 35, 255}
PINS = [
...
class TransportFailure(RuntimeError):
    """Only this explicit, bounded failure class can enter the retry loop."""

    def __init__(self, operation, reason, returncode=None):
        super().__init__(reason)
        self.operation, self.returncode = operation, returncode


def exit_failure(returncode, command, operation):
    allowed = RSYNC_TRANSPORT_CODES if operation == 'transfer' else {255}
    if returncode in allowed:
        return TransportFailure(operation, f'{operation} transport exit {returncode}', returncode)
...
def run_polled(command, stdout_path, stderr_path, guard, timeout, *, operation='transfer'):
    """Always clean the complete group before any retryable exception escapes."""
        ...
        try:
            while process.poll() is None:
                ...
                if time.monotonic() - started >= timeout:
                    if operation in ('status', 'transfer'):
                        raise TransportFailure(operation, f'Bounded {operation} timeout reached')
                    raise StopPrefetch('Bounded remote snapshot deadline reached')
                time.sleep(min(POLL_SECONDS, max(0.01, timeout - (time.monotonic() - started))))
            if process.returncode:
                raise exit_failure(process.returncode, command, operation)
            guard()
        except BaseException as error:
            failure = error
        finally:
            # Preserve an already completed semantic failure even if a concurrent
            # guard SSH timed out. Status produced by our later cancellation is
            # separate and cannot turn a transport timeout into a semantic exit.
            before_cleanup_returncode = process.poll()
            receipt = stderr_path.with_suffix(stderr_path.suffix + '.process.json')
            try:
                stop_process(process)
            except BaseException as error:
                io._new_json(receipt, {'operation': operation, 'pid': process.pid, 'pgid': process.pid,
                    'finished_utc': utc(), 'process_group_quiescent': False,
                    'before_cleanup_returncode': before_cleanup_returncode,
                    'cleanup_error_type': type(error).__name__, 'cleanup_error': str(error)})
                raise StopPrefetch('Subprocess group cleanup failed; transport retry forbidden') from error
            io._new_json(receipt, {'operation': operation, 'pid': process.pid, 'pgid': process.pid,
                'finished_utc': utc(), 'returncode': process.returncode,
                'before_cleanup_returncode': before_cleanup_returncode, 'process_group_quiescent': True})
        if failure is not None:
            observed_code = before_cleanup_returncode
            if (isinstance(failure, TransportFailure) and observed_code is None
                    and process.returncode is not None and process.returncode > 0
                    and not (operation == 'transfer' and process.returncode == 20)):
                # Conservatively retain a semantic positive exit that becomes
                # visible during cleanup. Negative signals and rsync20 can be
                # generated by our cancellation and are not semantic evidence.
                observed_code = process.returncode
            if isinstance(failure, TransportFailure) and observed_code not in (None, 0):
                observed = exit_failure(observed_code, command, operation)
                if not isinstance(observed, TransportFailure):
                    raise observed from failure
            raise failure


def resume_manifest(prior, expected):
    ...
    incomplete_record, _ = checked_record(incomplete_path)
    plan, incomplete = io._json(plan_path), io._json(incomplete_path)
    require(plan.get('lease_name') == NAME and plan.get('pod_id') == POD
    ...
    require(not (prior / 'PREFETCH_VERIFIED.json').exists() and not (prior / 'PREFETCH_VERIFIED.json').is_symlink(),
            'completed operations cannot be resumed')
    files = {}
    staging = io._no_links(prior / 'staging')
    require(staging.is_dir(), 'prior operation has no staging directory')
    for path in staging.rglob('*'):
        io._no_links(path)
        if path.is_dir():
            continue
        require(path.is_file(), 'nonregular prior staged member')
        relative = path.relative_to(staging).as_posix()
        require(relative in expected['binaries'], 'nonwhitelisted prior staged member')
        require(0 <= path.stat().st_size <= expected['binaries'][relative]['bytes'], 'prior staged file exceeds expected size')
        files[relative] = str(path)
    require(files, 'prior operation has no matching staged files to resume')
    return {'root': str(prior), 'plan': plan_record, 'incomplete': incomplete_record, 'source': source,
            'files': dict(sorted(files.items()))}


def copy_resume_file(source, staged, scratch, expected, guard=lambda: None):
    """Copy an opaque partial; equality to the full sealed file is checked later."""
    source, staged, scratch = map(io._no_links, (source, staged, scratch))
    require(source != staged and source != scratch and not staged.exists() and not scratch.exists(),
            'resume refuses existing new staging or scratch paths')
    before, old_signature = checked_record(source, guard)
    require(before['bytes'] <= expected['bytes'], 'old partial exceeds expected full size')
    source_stat = source.stat()
    staged.parent.mkdir(parents=True, exist_ok=True)
    scratch.parent.mkdir(parents=True, exist_ok=True)
    io._no_links(staged.parent); io._no_links(scratch.parent)
    digest, size, checked = hashlib.sha256(), 0, time.monotonic()
    with source.open('rb') as src, scratch.open('xb') as dst:
        require(signature(os.fstat(src.fileno())) == old_signature, 'old partial changed before copying')
        while block := src.read(8 * 1024 * 1024):
            dst.write(block); digest.update(block); size += len(block)
            if time.monotonic() - checked >= POLL_SECONDS:
                guard(); checked = time.monotonic()
        dst.flush(); os.fsync(dst.fileno())
        require(signature(os.fstat(src.fileno())) == old_signature, 'old partial changed during copying')
    require({'bytes': size, 'sha256': digest.hexdigest()} == before, 'copied partial differs from old bytes')
    after, after_signature = checked_record(source, guard)
    require(after == before and after_signature == old_signature, 'old partial changed after copying')
    copied, scratch_signature = checked_record(scratch, guard)
    require(copied == before, 'new partial copy differs from old bytes')
    os.utime(scratch, ns=(source_stat.st_atime_ns, source_stat.st_mtime_ns))
    scratch_signature = signature(scratch.stat())
    guard()
    io._no_links(source); io._no_links(staged); io._no_links(scratch)
    require(signature(source.stat()) == old_signature, 'old partial changed before reuse')
    require(signature(scratch.stat()) == scratch_signature and not staged.exists(), 'new partial changed or destination appeared')
    # This links our verified COPY only. The old source always has its own inode.
    os.link(scratch, staged, follow_symlinks=False)
    scratch.unlink()
    require(staged.stat().st_ino != source.stat().st_ino or staged.stat().st_dev != source.stat().st_dev,
            'resume must not share the old partial inode')
    return {'source': str(source), 'old_before': before, 'old_after': after, 'copied': copied,
            'old_signature': list(old_signature), 'copied_mtime_ns': staged.stat().st_mtime_ns,
            'meaning': 'Opaque partial rsync basis only; final complete size/SHA and remote mtime remain mandatory.'}


class Prefetch:
    def __init__(self, expected, operation, resume=None):
        ...
        self.state = None
        self.file_deadline = None
        self.attempt_dir = None
        self.file_anchor = None

    def remaining(self):
        if self.file_deadline is None:
            return TRANSFER_TIMEOUT
        remaining = self.file_deadline - time.monotonic()
        if remaining <= 0:
            raise StopPrefetch('Original per-file deadline reached; retries cannot extend it')
        return remaining
    ...
    def status(self, check_lock=True, timeout=STATUS_TIMEOUT):
        ...
        folder = (self.attempt_dir or self.operation) / 'status'
        folder.mkdir(exist_ok=True)
        stdout, stderr = folder / f'{self.status_counter:06d}.json', folder / f'{self.status_counter:06d}.stderr'
        # Status is also a managed group; a timed-out SSH can leave no children.
        run_polled(self.remote_command(), stdout, stderr, lambda: self.local_active(check_lock),
                   min(timeout, self.remaining()), operation='status')
    ...
    def snapshot(self, relative, label):
        self.status()
        stem = self.attempt_dir / label
        stdout, stderr = stem.with_suffix('.json'), stem.with_suffix('.stderr')
        run_polled(self.remote_command(relative), stdout, stderr, self.status,
                   min(HASH_TIMEOUT, self.remaining()), operation='snapshot')
        value = json.loads(stdout.read_text())
    ...
    def copy_prior_partial(self, relative, staged, expected):
        if self.resume is None or relative not in self.resume['files'] or staged.exists():
            return
        # Attempt-scoped scratch is never reused if copying is interrupted.
        result = copy_resume_file(Path(self.resume['files'][relative]), staged,
                                  self.attempt_dir / 'resume_copy.partial', expected, self.status)
        for filename, key in (('PLAN.json', 'plan'), ('INCOMPLETE.json', 'incomplete')):
            io._verify_record(Path(self.resume['root']) / filename, self.resume[key])
        io._new_json(self.operation / f'{self.counter:02d}_RESUMED.json', result)

    def transfer_command(self, relative, staged):
        transport = shlex.join(lease.ssh_options(LEASE_ROOT, self.state))
        return ['rsync', '-rlt', '--partial', '--timeout=60', '-e', transport,
                'root@' + self.state['ssh']['host'] + ':' + REMOTE + '/' + relative, str(staged)]

    def attempt_file(self, relative, expected):
        # A new successful full status/gate check is mandatory for EVERY attempt.
        self.status()
        before = self.snapshot(relative, 'before')
        if self.file_anchor is None:
            self.file_anchor = before['binary']
        require(before['binary'] == self.file_anchor, 'remote binary or mtime changed between transport attempts')
        target = io._no_links(LEASE_ROOT / 'retrieved' / relative)
        staged = io._no_links(self.operation / 'staging' / relative)
        staged.parent.mkdir(parents=True, exist_ok=True)
        if target.exists():
            ...
            self.copy_prior_partial(relative, staged, expected)
            run_polled(self.transfer_command(relative, staged), self.attempt_dir / 'rsync.log',
                       self.attempt_dir / 'rsync.stderr', self.status, self.remaining(), operation='transfer')
            after = self.snapshot(relative, 'after')
    ...
    def run_file(self, relative, expected):
        self.file_deadline = time.monotonic() + TRANSFER_TIMEOUT
        self.file_anchor = None
        deadline_utc = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(seconds=TRANSFER_TIMEOUT)
        io._new_json(self.operation / f'{self.counter:02d}_FILE.json', {
            'path': relative, 'record': expected, 'started_utc': utc(), 'deadline_utc': deadline_utc.isoformat(),
            'maximum_attempts': MAX_ATTEMPTS, 'overall_budget_seconds': TRANSFER_TIMEOUT})
        try:
            for attempt in range(1, MAX_ATTEMPTS + 1):
                self.remaining()
                self.attempt_dir = self.operation / f'{self.counter:02d}_attempt_{attempt:02d}'
                self.attempt_dir.mkdir(exist_ok=False)
                io._new_json(self.attempt_dir / 'STARTED.json', {'path': relative, 'attempt': attempt,
                    'started_utc': utc(), 'remaining_seconds': self.remaining(), 'deadline_utc': deadline_utc.isoformat()})
                try:
                    result = self.attempt_file(relative, expected)
                except BaseException as error:
                    # run_polled has already cleaned/reaped every group before
                    # reaching this handler. A failed cleanup raises StopPrefetch.
                    retryable = isinstance(error, TransportFailure)
                    retry = retryable and attempt < MAX_ATTEMPTS and self.file_deadline > time.monotonic()
                    io._new_json(self.attempt_dir / 'FAILED.json', {'path': relative, 'attempt': attempt,
                        'finished_utc': utc(), 'error_type': type(error).__name__, 'reason': str(error),
                        'transport_operation': getattr(error, 'operation', None),
                        'returncode': getattr(error, 'returncode', None), 'transport_retryable': retryable,
                        'retry_scheduled': retry,
                        'cleanup_rule': 'Each started subprocess has a separate process-group quiescence receipt; failed cleanup forbids retry.',
                        'deadline_utc': deadline_utc.isoformat()})
                    if not retry:
                        raise
                    print(json.dumps({'event': 'transport_retry', 'path': relative, 'failed_attempt': attempt,
                                      'next_attempt': attempt + 1, 'remaining_seconds': self.remaining()}), flush=True)
                    continue
                io._new_json(self.attempt_dir / 'PASSED.json', {**result, 'attempt': attempt})
                return result
            raise StopPrefetch('Maximum transport attempts exhausted')
        finally:
            self.file_deadline = None
            self.attempt_dir = None
            self.file_anchor = None
    ...
    def run(self):
        ...
        for relative, expected in self.expected['binaries'].items():
            ...
            result = self.run_file(relative, expected)
...
def main(argv=None):
    ...
    parser.add_argument('--resume-from', type=Path, help='Read an incomplete pinned prior operation; copy matching partials into a new operation')
    args = parser.parse_args(argv)
    expected = expected_inventory()
    resume = resume_manifest(args.resume_from, expected) if args.resume_from is not None else None
    ...
    if resume is not None:
        io._new_json(operation / 'RESUME_PLAN.json', resume)
    runner = Prefetch(expected, operation, resume)
    try:
        rows = runner.run()
    except BaseException as error:
