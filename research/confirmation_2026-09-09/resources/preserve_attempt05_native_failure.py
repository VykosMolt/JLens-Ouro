"""Preserve the terminal attempt-05 native-gate failure before teardown."""
...
import artifact_handoff as handoff  # noqa: E402
import lease  # noqa: E402
from transport import run_bounded  # noqa: E402

root = ROUND / "cloud_leases/attempt_05"
...
assert "development/native.pt" in state["computation_finished"]["manifest"]["files"]
...
def read(p):
 ...
 before=p.stat();assert stat.S_ISREG(before.st_mode) and before.st_size<=32*1024**2
 with p.open('rb') as f:
  assert sig(os.fstat(f.fileno()))==sig(before);b=f.read();assert sig(os.fstat(f.fileno()))==sig(before)
 assert sig(p.stat())==sig(before)
 return {'bytes':len(b),'sha256':hashlib.sha256(b).hexdigest(),'base64':base64.b64encode(b).decode()}
files={}
for rel in ('provider_lease_name','status.json','bootstrap_stdout.log','worker_config.json','BANKS_READY.json',
            'artifact_index.json','bundle/evaluation/worker.py','bundle/frozen/run_spec.json',
            'bundle/frozen/output_contract.json'):
 p=r/rel
...
raw = run_bounded([*lease.ssh_options(root, state), "root@" + state["ssh"]["host"],
                  ...
                  operation="preserve_attempt05_native_failure").stdout
...
out = {
    "schema": "confirmation_native_gate_failure_preservation.v1",
    "recorded_utc": datetime.now(timezone.utc).isoformat(),
    "binding": handoff.binding(state),
    "failed_final": state["computation_finished"],
    "remote": evidence,
    "note": "Terminal exact-native-equality failure before external development acceptance or new-item scoring.",
}
path = ROUND / "resources/ATTEMPT05_NATIVE_FAILURE_PRESERVED.json"
