...
Only recognized explicit-B / flattened-B*T dense strided tensors whose every
lane is bytewise equal are compressed. Axis zero is preferred when dimensions
are ambiguous; other ambiguities are skipped. Parameters/buffers are excluded by
...
class ReplicatedSavedTensorHooks:
    ...
    def _layout(self, tensor: torch.Tensor) -> tuple[int, bool] | None:
        ...
        # B can equal the head count, or B*T can equal D. A leading candidate
        # remains safe: reconstruction is identity along ANY selected axis if
        # every lane has identical bits. Equality below proves this directly;
        # semantic batch-axis inference is unnecessary for value preservation.
        leading = next((candidate for candidate in candidates if candidate[0] == 0), None)
...
def _self_test() -> dict[str, Any]:
    ...
    assert isinstance(compressor.pack(torch.ones(3, 3, 7)), PackedLane)
    assert isinstance(compressor.pack(torch.ones(7, 3, 3)), torch.Tensor)
    ...
    # Actual Ouro ambiguity sizes: B equals the 16 attention heads, while B*T
    # equals width 2048. Each full BF16 tensor here occupies only 8 MiB on CPU.
    ambiguous_compressor = ReplicatedSavedTensorHooks(holder, batch_size=16, seq_len=128, min_savings_bytes=1)
    attention = torch.randn(1, 16, 128, 128, dtype=torch.bfloat16).repeat(16, 1, 1, 1)
    flattened = torch.randn(1, 128, 2048, dtype=torch.bfloat16).repeat(16, 1, 1).reshape(2048, 2048)
    ambiguous_checks = []
    for name, original in (("B16_H16_T128_D128", attention), ("BT2048_D2048", flattened)):
        payload = ambiguous_compressor.pack(original)
        assert isinstance(payload, PackedLane) and payload.axis == 0
        restored = ambiguous_compressor.unpack(payload)
        assert _bitwise_equal(restored, original)
        assert restored.stride() == original.stride() and restored.storage_offset() == original.storage_offset()
        different = original.clone()
        different[tuple(size - 1 for size in different.shape)] = float("inf")
        assert isinstance(ambiguous_compressor.pack(different), torch.Tensor)
        ambiguous_checks.append({"layout": name, "shape": list(original.shape), "axis": payload.axis, "flattened": payload.flattened, "value_bits_and_layout_exact": True, "nonidentical_lane_skipped": True})
    ...
    return {"device": "cpu", "torch_version": torch.__version__, "roundtrips": roundtrips, "ambiguous_axis_checks": ambiguous_checks, "original_tensor_and_storage_released": True, "nan_payload_and_signed_zero_guards": "pass", "full_jacobian_parity": parity, "exception_cleanup": "pass", "cuda_initialized": torch.cuda.is_initialized()}
