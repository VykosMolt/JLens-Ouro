...
RECEIVE_CHUNK_BYTES = 64 * 1024 * 1024
RECEIVE_STREAMS = 8
RECEIVE_RANGE_PROGRAM = r'''
import json,os,pathlib,stat,sys,time
request=json.loads(__REQUEST__)
WORK=pathlib.Path('/workspace/jlens')
def no_links(path):
 for parent in (path,*path.parents):
  if parent.is_symlink():raise ValueError('linked remote range path')
 return path
def guard():
 if no_links(WORK/'provider_lease_name').read_text().strip()!=request['lease_name']:raise ValueError('wrong remote lease marker')
 if time.time()>=request['deadline']:raise ValueError('fixed retrieval deadline reached')
 # WORK/STOP stops scientific computation; preservation continues to deadline.
def sig(s):return s.st_dev,s.st_ino,s.st_size,s.st_mtime_ns,s.st_ctime_ns
guard();name=request['name']
if name.startswith('/') or any(p in ('','.','..') for p in name.split('/')) or '\\' in name:raise ValueError('unsafe remote range path')
p=no_links(WORK/'results'/name);before=p.stat()
if not stat.S_ISREG(before.st_mode) or before.st_size!=request['bank_record']['bytes']:raise ValueError('remote range source geometry differs')
offset,length=request['offset'],request['length']
if type(offset)is not int or type(length)is not int or offset<0 or not 0<length<=64*1024*1024 or offset+length>before.st_size:raise ValueError('invalid remote range')
blocks=[];remaining=length
with p.open('rb') as reader:
 if sig(os.fstat(reader.fileno()))!=sig(before):raise ValueError('remote range descriptor differs before read')
 reader.seek(offset)
 while remaining:
  guard();block=reader.read(min(8*1024*1024,remaining))
  if not block:raise ValueError('short remote range')
  blocks.append(block);remaining-=len(block)
 if sig(os.fstat(reader.fileno()))!=sig(before):raise ValueError('remote range descriptor changed while reading')
if sig(no_links(p).stat())!=sig(before):raise ValueError('remote range pathname changed')
guard()
for block in blocks:sys.stdout.buffer.write(block)
...
class SSHTransport:
    def __init__(self, root, state, ssh_options):
        ...
        self.deadline = min(time.time() + state['transfer_timeout_seconds'],
                            datetime.fromisoformat(state['watch_deadline_utc'].replace('Z', '+00:00')).timestamp())

    def _python(self, script, *, timeout, operation):
        timeout = min(timeout, self.deadline - time.time())
        require(timeout > 0, 'fixed retrieval deadline reached')
...
""" % encoded
    ...
    def _range_command(self, script):
        return [*self.options, 'root@' + self.state['ssh']['host'], 'python -c ' + shlex.quote(script)]

    def transfer(self, files, destination, expected_records):
        """Eight bounded range readers; full manifest SHA remains publication authority."""
        destination = io._no_links(destination)
        partial = io._mkdir(destination.parent / 'partials' / 'chunks')
        cancelled, lock = threading.Event(), threading.RLock()
        state_identity = binding(self.state)
        deadline = min(self.deadline, time.time() + self.state['transfer_timeout_seconds'])
        def kill(entry):
        ...
        def cancel():
            with lock:
                ...
                for entry in records:
                    kill(entry)
        def guard():
            require(not cancelled.is_set(), 'retrieval cancelled')
            current = io._json(io._no_links(self.root / 'LEASE.json'))
            require(binding(current) == state_identity
                    and current.get('ssh') == self.state.get('ssh')
                    and current.get('ssh_identity') == self.state.get('ssh_identity')
                    and current.get('watch_deadline_utc') == self.state.get('watch_deadline_utc')
                    and current.get('provider_deadline_utc') == self.state.get('provider_deadline_utc'),
                    'retrieval lease identity or deadline changed')
            require(not current.get('halt_requested') and not current.get('retrieval_halt_requested')
                    and current.get('status') not in ('terminating', 'terminated'), 'retrieval halted')
            # Worker STOP and work_deadline halt computation, not preservation.
            require(time.time() < deadline, 'fixed retrieval deadline reached')
        ...
        def chunk_metadata(bank, offset, length, pin):
            return {'schema': 'confirmation_received_chunk.v1', 'bank_record': bank,
                    'offset': offset, 'length': length, 'record': pin}
        def receive(name, bank, offset, length, cache):
            guard()
            stem = f'{offset:012d}'
            part, meta = io._no_links(cache / (stem + '.part')), io._no_links(cache / (stem + '.json'))
            if part.exists():
                existing = io._json(meta)
                require(existing == chunk_metadata(bank, offset, length, valid_record(existing['record']))
                        and existing['record']['bytes'] == length, 'cached chunk binding changed')
                verify_record(part, existing['record'])
                return part, existing['record']
            request = {'name': safe_path(name), 'bank_record': bank, 'offset': offset, 'length': length,
                       'lease_name': self.state['name'], 'deadline': deadline}
            script = RECEIVE_RANGE_PROGRAM.replace('__REQUEST__', repr(json.dumps(request, sort_keys=True)))
            result = run_bounded(self._range_command(script), timeout=guard(),
                                 operation='receive_range:' + name + ':' + str(offset), processes=records)
            guard()
            data = result.stdout
            require(len(data) == length, 'remote range returned a different byte count')
            pin = {'bytes': length, 'sha256': hashlib.sha256(data).hexdigest()}
            value = chunk_metadata(bank, offset, length, pin)
            if meta.exists():
                require(io._json(meta) == value, 'resumed remote range changed')
            else:
                io._new_json(meta, value)
            temporary = io._no_links(cache / (stem + '.receiving-' + uuid.uuid4().hex))
            try:
                with temporary.open('xb') as writer:
                    writer.write(data); writer.flush(); os.fsync(writer.fileno())
                guard(); verify_record(temporary, pin)
                os.link(temporary, part, follow_symlinks=False)
                io._fsync_dir(cache)
            finally:
                temporary.unlink(missing_ok=True)
            return part, pin
        try:
            guard(); verify_local_sources(self.state)
            for name in files:
                guard(); name = safe_path(name); bank = valid_record(expected_records[name])
                target = io._no_links(destination / name)
                if target.exists() and checked_record(target) == bank:
                    continue
                cache = io._mkdir(partial / bank['sha256'])
                identity = {'schema': 'confirmation_received_bank.v1', 'record': bank,
                            'chunk_bytes': RECEIVE_CHUNK_BYTES}
                if (cache / 'BANK.json').exists():
                    require(io._json(cache / 'BANK.json') == identity, 'cached bank identity changed')
                else:
                    io._new_json(cache / 'BANK.json', identity)
                chunks, pending = {}, {}
                offsets = iter(range(0, bank['bytes'], RECEIVE_CHUNK_BYTES))
                with ThreadPoolExecutor(max_workers=RECEIVE_STREAMS) as executor:
                    try:
                        def schedule():
                            while len(pending) < RECEIVE_STREAMS:
                                offset = next(offsets, None)
                                if offset is None:
                                    return
                                guard()
                                future = executor.submit(receive, name, bank, offset,
                                                         min(RECEIVE_CHUNK_BYTES, bank['bytes']-offset), cache)
                                pending[future] = offset
                        schedule()
                        while pending:
                            guard()
                            done, _ = wait(pending, timeout=min(.1, max(.001, guard())), return_when=FIRST_COMPLETED)
                            for future in done:
                                chunks[pending.pop(future)] = future.result()
                            schedule()
                    except BaseException:
                        cancel()
                        for future in pending:
                            future.cancel()
                        raise
                guard(); io._mkdir(target.parent)
                temporary = io._no_links(target.parent / ('.' + target.name + '.assembling-' + uuid.uuid4().hex))
                try:
                    with temporary.open('x+b') as writer:
                        digest_value, count = hashlib.sha256(), 0
                        for offset in sorted(chunks):
                            guard(); part, pin = chunks[offset]
                            before = part.stat(); one, length = hashlib.sha256(), 0
                            with part.open('rb') as reader:
                                require(signature(os.fstat(reader.fileno())) == signature(before), 'chunk descriptor changed before assembly')
                                while block := reader.read(8 * 1024 * 1024):
                                    guard(); writer.write(block); one.update(block); digest_value.update(block)
                                    length += len(block); count += len(block)
                                require(signature(os.fstat(reader.fileno())) == signature(before), 'chunk changed during assembly')
                            require(signature(io._no_links(part).stat()) == signature(before)
                                    and {'bytes': length, 'sha256': one.hexdigest()} == pin, 'chunk pathname or hash changed')
                        require({'bytes': count, 'sha256': digest_value.hexdigest()} == bank, 'received full manifest SHA differs')
                        writer.flush(); os.fsync(writer.fileno())
                        sealed = os.fstat(writer.fileno())
                        require(signature(io._no_links(temporary).stat()) == signature(sealed), 'assembly descriptor/path differ')
                        writer.seek(0); full, count = hashlib.sha256(), 0
                        while block := writer.read(8 * 1024 * 1024):
                            guard(); full.update(block); count += len(block)
                        require(signature(os.fstat(writer.fileno())) == signature(sealed)
                                and {'bytes': count, 'sha256': full.hexdigest()} == bank, 'assembly changed before publication')
                        guard()
                        require(signature(io._no_links(temporary).stat()) == signature(sealed), 'assembly pathname changed before publication')
                        os.replace(temporary, target)
                        published = io._no_links(target).stat()
                        require((published.st_dev,published.st_ino,published.st_size,published.st_mtime_ns)
                                == (sealed.st_dev,sealed.st_ino,sealed.st_size,sealed.st_mtime_ns), 'publication inode changed')
                        io._fsync_dir(target.parent)
                    guard(); verify_record(target, bank)
                    # Cache is receiver-owned and removed only after whole-file verification.
                    for part, _ in chunks.values():
                        part.unlink()
                        part.with_suffix('.json').unlink()
                    io._fsync_dir(cache)
                finally:
                    temporary.unlink(missing_ok=True)
        finally:
