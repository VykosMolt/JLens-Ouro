...
FROZEN = {
    ...
    'corrections/native_check_v1/bundle/frozen/run_spec.json': {'bytes': 34693, 'sha256': '002b91cb755a0517fc5839ef2277f14307ef9eaba41176b9298f4362e040d446'},
...
def verify(results, analysis, records, checks):
    ...
    for name in ('run_spec', 'population', 'benchmark'):
        checks.exact('results.' + name + '.frozen_record', records[str(regular(results / (name + '.json')))], FROZEN[('corrections/native_check_v1/bundle/frozen/' if name == 'run_spec' else 'bundle/frozen/') + name + '.json'])
