...
def main():
    ...
    try:
        ...
        run('packages',[str(python),'-m','pip','install','--no-deps','--no-cache-dir','--require-hashes','--only-binary=:all:','-r',str(bundle/'legacy/requirements.lock')])
        run('model',[str(python),str(bundle/'evaluation/download_model.py'),'--manifest',str(bundle/'legacy/model_manifest.json'),'--out',config['snapshot']])
        status('awaiting_verified_bank_uploads')
        ready=work/'BANKS_READY.json'
        while not ready.exists():check();time.sleep(2)
        ack=json.loads(ready.read_text())
        if ack.get('binding')!=binding:raise ValueError('Wrong input upload binding')
        for entry in spec['bank_records'].values():
            check()
            if record(work/entry['worker_path'])!=entry['record']:raise ValueError('Uploaded bank differs')
