...
def main():
    ...
    if args.phase=='start':
        ...
        config={'work':'/workspace/jlens','bundle':'/workspace/jlens/bundle','snapshot':'/workspace/jlens/models/ouro',
        ...
        if remaining()<=60:raise ValueError('Insufficient admitted setup time')
        handoff.verify_record(Path(state['bundle']),state['bundle_record'])
        started=local/'BOOTSTRAP_STARTED.json'
        if not started.exists():
            upload(Path(state['bundle']),'/workspace/jlens/bundle.tar.gz',min(300,remaining()))
        ...
        remote('mkdir -p /workspace/jlens/banks')
        seen=set()
        for entry in spec['bank_records'].values():
            if entry['worker_path'] in seen:continue
            seen.add(entry['worker_path'])
            state_now=json.loads((root/'LEASE.json').read_text())
            if handoff.binding(state_now)!=binding or state_now.get('halt_requested') or state_now['status']=='terminated':raise ValueError('Lease stopped or changed during upload')
            source=Path(entry['receiver_path']);handoff.verify_record(source,entry['record'])
            if remaining()<=30:raise ValueError('Input upload exceeded admitted setup window')
            upload(source,'/workspace/jlens/'+entry['worker_path'],remaining())
            print(json.dumps({'uploaded_bank':entry['worker_path'],'record':entry['record']}),flush=True)
        ready=local/'BANKS_READY.json'
        if not ready.exists():json_save(ready,{'binding':binding,'bank_records':spec['bank_records']})
        upload(ready,'/workspace/jlens/BANKS_READY.json.upload',min(60,remaining()))
        remote('mv /workspace/jlens/BANKS_READY.json.upload /workspace/jlens/BANKS_READY.json')
        ...
        print(json.dumps({'status':'development_accepted_and_confirmation_authorized','manifest_sha256':handoff.digest(manifest),'local_receipt':pointer}))
