...
In one frozen recurrent model, one fixed Jacobian-lens estimator recovers annotated intermediate concepts from a fixed final-pass band far more often than the raw logit lens, and this transferred to 160 new questions without refitting or band selection. In the same physical layers of the three earlier passes the raw lens is better. Nothing here is a monitoring system, a deception detector, a chain-of-thought baseline, or evidence that the model uses the recovered content.
...
::: {.resultbox title="Descriptive observation --- own-exit targets read early passes far better, on the discovery population"}

The application-era banks were all applied to the 148 discovery items in one evaluation whose rank arrays survive. On the 90 eligible multihop items, in the fixed band 26–37, the lens targeting each pass's own exit recovers the intended concept far more often than both alternatives: intended recovery 15.4%, 16.2% and 25.2% in passes 1–3, against 2.1%, 4.3% and 10.1% for the raw lens and 0.3%, 0.4% and 2.1% for the final-target lens (Figure 5). Own exit minus final target: +14.6, +16.9 and +23.0 points (shared-concept-cluster percentile intervals 9.1–20.9, 11.5–22.8, 16.0–30.6); own exit minus raw: +13.0, +11.3 and +14.9 (8.1–19.0, 6.6–16.6, 7.9–22.4). Control recovery stays below 2.3%. This is descriptive, post hoc, on the population used to select the band, with the older calibration prefix, and untested on the confirmation population.

:::

![Local-exit discovery comparison](figures_v3/fig5_local_exit.pdf)

\kirinfigcaption{Figure 5.}{Application-era matched banks (same recipe, target differs) evaluated on the discovery population, fixed layers, passes 1–3: raw lens (grey), final-target J-Lens (red) and the pass's own-exit J-Lens (teal). (d) Band means with descriptive shared-concept-cluster 95\% intervals. Post hoc and descriptive; not the confirmation population.}

**Interpretation.** Within the rules fixed before these numbers were computed, this is the case in which the local target beats both alternatives: a target-dependent early-pass advantage, consistent with a transport explanation. It is not proof that averaging destroyed information. Two existing qualifications remain: shortening the final target by one block (the penultimate control) reduced the late gain, so transport distance alone is not established, and raw pass counts are not comparable transport lengths across architectures. The historical Huginn pilot (Appendix A) did not identify a cause or rule out supervision-based explanations.
...
| The gain is concept-specific rather than domain-level | \statusBounded | Within-domain controls remove about a ninth of the gain; heterogeneity across domains is large |
...
| Own-exit targets read early passes better | \statusDiagnostic | Descriptive, discovery population only; confirmation-population test blocked |
...
| All historical estimator binaries were preserved | \statusRetracted | Four FP32 checkpoints, the Huginn estimator and the local-exit banks are lost or remote-only |
